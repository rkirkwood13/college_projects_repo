		...
		switch(right_button_counter) {
			...
			case 12: //case to run the commands
				...
				case 2: //second command run
					pwm_counter_m1 = m1_direction(pwm_counter_m1, direction2, speed2);
					pwm_counter_m2 = m2_direction(pwm_counter_m2, direction2, speed2);
				...
				case 3: //third command run
					pwm_counter_m1 = m1_direction(pwm_counter_m1, direction3, speed3);
					pwm_counter_m2 = m2_direction(pwm_counter_m2, direction3, speed3);
					time_counter= time_counter + 1;
				...
				case 4: //fourth command run
					pwm_counter_m1 = m1_direction(pwm_counter_m1, direction4, speed4);
					pwm_counter_m2 = m2_direction(pwm_counter_m2, direction4, speed4);
					time_counter= time_counter + 1;
...
int m1_backwards(unsigned int pwm_counter, int duty_cycle){
  ...
  if( pwm_counter < duty_cycle ){
    ...
    PORTD &= ~(1<<6);
		...
		PORTD |= (1<<6); //Turn on both ports to brake
    PORTD |= (1<<5);
    //PORTD &= ~(1<<6); //Turn off both ports to coast
    //PORTD &= ~(1<<5);
  }
	return pwm_counter;
}

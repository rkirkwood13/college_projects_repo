	...
	while(1){
		...
		switch(sensor_counter){
		  ...
		  case 3: //bunker may have been found, move forward
		    direction_time(F, SLOW, 200);
				direction_time(F, 0, 100);
...
void turn_right(int time){
	direction_time(CW, SLOW, time); //Turns CW for 10 ms
}

void turn_left(int time){
	direction_time(CCW, SLOW, time); //Turns CCW for 10 ms
}
